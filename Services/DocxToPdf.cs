...
public class DocxToPdfService
{
    ...
    // {
    //     using var docx = WordprocessingDocument.Open(docxStream, true);
    ...
    //     var imgs = docx.MainDocumentPart?.ImageParts;

    //     if (imgs != null)
    //     {
    //         foreach (var img in imgs)
    //         {
    //             var imgId = docx.MainDocumentPart?.GetIdOfPart(img);
    //             using var stream = img.GetStream();
    //             using var memoryStream = new MemoryStream();
    //             stream.CopyTo(memoryStream);

    //             var base64 = Convert.ToBase64String(memoryStream.ToArray());
    //             var imgExtension = img.ContentType;
    //             var dataUri = $"data:{imgExtension};base64,{base64}";

    //             htmlContent = htmlContent.Replace($"cid:{imgId}", dataUri);
    //         }
    //     }
